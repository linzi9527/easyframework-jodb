...
public class DBMasterSlaveHelper2 {
	private static final Logger log = LoggerFactory.getLogger(DBMasterSlaveHelper2.class);
	
	
	private static final DBMasterSlaveHelper2 postDB = new DBMasterSlaveHelper2();
	private Connection connection = null;
	private Connection connectionMaster = null;
	//private String[] link_name = null;

	public static DBMasterSlaveHelper2 getInstance() {
		return postDB;
	}
	...
	public ResultSet executeQuery(String sql) {
		...
		try {
			if (connection == null || connection.isClosed()) {
				...
				if(LoadPropertiesUtils2.Is_MS_OPEN&&LoadPropertiesUtils2.db_Master_W){
					connection =ConnUtils2.initConnection();
					if(null==connection){connection =ConnUtils2.initConnection();}
				}
			}
		...
		try {
			try {
				if (connectionMaster == null || connectionMaster.isClosed()) {
					//connection = ConnectionFactory.getInstance().getConnection();
					if(LoadPropertiesUtils2.Is_MS_OPEN&&LoadPropertiesUtils2.db_Master_W){
						connectionMaster =MasterConnectionFactory_db2.getInstance().getConnection();
					}
				}
