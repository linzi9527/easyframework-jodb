...
public class DBMasterSlaveHelper {
	...
	private static final DBMasterSlaveHelper postDB = new DBMasterSlaveHelper();
	private Connection connection = null;
	private Connection connectionMaster = null;

	// private String[] link_name = null;

	public static DBMasterSlaveHelper getInstance() {
		return postDB;
	}
	...
	public ResultSet executeQuery(String sql) {
		...
		try {
			if (connection == null || connection.isClosed()) {
				...
				if (LoadPropertiesUtils.Is_MS_OPEN
						&& LoadPropertiesUtils.db_Master_W) {
					connection = ConnUtils.initConnection();
					if (null == connection) {
						connection = ConnUtils.initConnection();
					}
				}
	...
	public int findCount(Class<?> o) {
		...
		try {
			...
			try {
				boolean tbl = o.newInstance().getClass()
						.isAnnotationPresent(Table.class);
				if (tbl) {
					Table t = (Table) o.newInstance().getClass()
	...
	public static List<?> querys(Class<?> o, boolean isCache)
		...
		try {
			...
			if (null != connection) {
				...
				if (tbl) {
					...
					// table = connection.getCatalog() + "." + t.name();
					tableName = t.name();
					// n = t.name().substring(t.name().indexOf("_") + 1,
					// t.name().indexOf("_") + 2).toLowerCase();
				...
				if (!StringUtil.isEmpty(tableName)
					...
					if (isWhere.trim().toLowerCase().startsWith("select")
							&& index > 0) {
						if (x > 0) {
							sb.append(isWhere.trim().substring(index + 5,
									isWhere.trim().length()));
						} else {
							sql = isWhere;
						}
			...
			// e.printStackTrace();
			log.error("SQLException-->" + e.getMessage());
			log.error("List<?> queryList方法SQLException格式语法缺少关键字异常："
					+ sql
					+ "\n"
					+ "---------------------query方法调用-系统提示-----------------------"
				...
				if (!StringUtil.isNull(sql)) {
					...
					if (sql.trim().toLowerCase().startsWith("select")) {
						...
						if (sql.trim().toLowerCase().indexOf("from") > 0
								&& sql.trim().toLowerCase().indexOf("where") > 0) {
							String ss = sql.trim().toLowerCase();
							String tbl_name = ss
									.substring(ss.indexOf("from") + 4,
											ss.indexOf("where"));
						...
						log.error("query方法SQLException：没有写select * from ...格式语法缺少关键字异常："
								+ sql
								+ "\n"
								+ "---------------------query方法调用-系统提示-----------------------"
								+ "提示，正确格式e.g:select *或字段名 from 表名 where 条件"
	...
	/**
	 * 自定义多表联合查询sql语句 必须设置那几张表关联 setTableLink e.g: String sql=
	 ...
	 * @return
	 */
	public List<?> queryTables(Class<?> o, String[] link_name, String sql,
			boolean isCache) {
		List list = null;
		SqlCommonDAO helper = new SqlCommonDAO();
		try {
			...
			if (null != connection) {
				if (!StringUtil.isNull(sql)) {
					...
					c.setSql(sql);
					if (sql.trim().toLowerCase().startsWith("select")
							&& null != link_name && link_name.length > 0) {
						Result rs = helper.executeQuery();
						if (rs != null && rs.getRowCount() > 0)
							list = JDBCBeanUtil.transTOList(rs, o);
					} else {
						log.error("queryTables方法多表联合操作调用异常："
								+ sql
								+ "\n"
		...
		try {
			try {
				if (connectionMaster == null || connectionMaster.isClosed()) {
					...
					if (LoadPropertiesUtils.Is_MS_OPEN
							&& LoadPropertiesUtils.db_Master_W) {
						connectionMaster = MasterConnectionFactory
				...
				if (fields.length > 1) {
					for (Field f : fields) {
						...
						if (flag) {
							...
							} else if (tp.endsWith("Timestamp".toUpperCase())) {
								name.append(c.columName()).append(",");
								value.append("NOW(),");
							} else if (tp.endsWith("BOOLEAN")
									|| tp.endsWith("LONG")) {
								name.append(c.columName()).append(",");
								value.append(v).append(",");
							} else if (tp.endsWith("DOUBLE")
									|| tp.endsWith("FLOAT")) {
								name.append(c.columName()).append(",");
								value.append(v).append(",");
							} else {
					...
					nameColum = name.toString().substring(0,
							name.toString().length() - 1);
					valueColum = value.toString().substring(0,
	...
	public String saveForKey(Object obj) throws Exception {
		...
		try {
			...
			try {
				helper.setConnection(connectionMaster);
				/*
				 * Command c = new Command(); helper.setCommand(c);
				 * c.setCache(false); c.setTables(new String[] { tbl_name });
				 * c.setSql(sb.toString());
				 */
				newID = helper.executeUpdateForKey(sb.toString(),
						connectionMaster);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				log.error("入库使用缓存异常：" + sb.toString(), e);
	 ...
	 */
	@SuppressWarnings("static-access")
	public boolean saveByTransaction(Object[] obj,String tableName) {
		int count = 0;
		SqlCommonDAO helper = new SqlCommonDAO();
		...
		try {
			...
			for (int index = 0; index < obj.length; index++) {
				...
				if (null==tbl_name&&tbl) {
					Table t = (Table) obj[index].getClass().getAnnotation(Table.class);
					tbl_name = t.name();
					if(!tableName.equals(tbl_name)){tbl_name=tableName;}
				}
			...
			if (count == obj.length) {
				all_sql.append(tbl_name).append(tbl_name_feilds).append(all_values);
	...
	public boolean removeByTransaction(String[] ids, Class<?> o) {
		...
		try {
			if (o != null) {
				...
				boolean t = o.newInstance().getClass()
						.isAnnotationPresent(Table.class);
				// 获得类是否有注解
				if (t) {
					Table tab = o.newInstance().getClass()
							.getAnnotation(Table.class);
					sql += tab.name();
					tbl_name = tab.name();
				}
				...
				try {
					...
					if (connection == null || connection.isClosed()) {
						...
						if (LoadPropertiesUtils.Is_MS_OPEN
								&& LoadPropertiesUtils.db_Master_W) {
							connection = MasterConnectionFactory.getInstance()
									.getConnection();
						} else {
							log.error("提示:Is_MS_OPEN、db_Master_W的参数重新设置");
						}
					}
					if (null != connection) {
						// 自动事物提交关闭，交由程序自行操作处理
							...
							if (!bl && isKey.isPrimary()) {// 判断是否存在主键
								...
								if (c != null && tp.endsWith("date")) {
									if (tobj != null) {
										...
										&& tp.endsWith("timestamp")) {
									if (tobj != null) {
										wh += fl.getName() + "=NOW(),";
									}
								} else if (c != null
										&& tp.equals(c.columName()
												.toLowerCase())) {
									if (tp.endsWith("string"))
										wh += fl.getName() + "='"
												+ tobj.toString() + "',";
	...
	public boolean exeCUDByTransaction(Object addObj, Object updateObj,
		...
		try {
			if (connectionMaster == null || connectionMaster.isClosed()) {
				...
				if (LoadPropertiesUtils.Is_MS_OPEN
						&& LoadPropertiesUtils.db_Master_W) {
					connectionMaster = MasterConnectionFactory.getInstance()
							.getConnection();
				}
			}
			// 自动事物提交关闭，交由程序自行操作处理
		...
		try {
			if (obj != null) {
				...
				try {
					...
					c.setTables(new String[] { tbl_name });
					c.setSql(sql.toString());
					i = helper.executeUpdates(connectionMaster);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public boolean delete(Integer id, Class<?> o, Connection connection) {
		 ...
		 * ConnectionFactory.getInstance().getConnection(); } } catch
	...
	private boolean deleteOfTransaction(String id, Class<?> o,
			SqlCommonDAO helper) {
		String tbl_name = "";
		boolean flag = false, flag_is = false;
		int i = 0;
	...
	private boolean deleteOfTransaction(Integer id, Class<?> o,
			SqlCommonDAO helper) {
		String tbl_name = "";
		boolean flag = false, flag_is = false;
		int i = 0;
	...
	public boolean deleteByTransaction(Integer id, Class<?> o, Integer sid,
		...
		try {
			...
			if (k1)
				k2 = this.deleteOfTransaction(sid, so, helper);
			if (k1 && k2) {
	...
	public Object get(Integer id, Class<?> o, boolean isCache) {
		...
		try {
			...
			Field[] fiels = o.newInstance().getClass().getDeclaredFields();// 获得反射对象集合
			boolean tbl = o.newInstance().getClass()
					.isAnnotationPresent(Table.class);// 获得类是否有注解
			if (tbl) {
				Table t = (Table) o.newInstance().getClass()
						.getAnnotation(Table.class);
				table = t.name();
				n = t.name()
						.substring(t.name().indexOf("_") + 1,
								t.name().indexOf("_") + 2).toLowerCase();
				sb.append(table).append(" " + n + "  ");
				// sb.append(table);
			}
			for (Field fl : fiels) {// 循环组装
				...
				if (isKey != null) {
					if (!flag_is && isKey.isPrimary()) {
						...
						sb.append("  WHERE  ").append(n).append(".")
								.append(fl.getName()).append("=").append(id);
						break;
					}
				}
			}
	...
	public Object load(Class<?> o, String iswhere, boolean isCache) {
		...
		try {
			isTable = o.newInstance().getClass()
					.isAnnotationPresent(Table.class);
		...
		try {
			...
			if (isTable) {
				...
				if (!StringUtil.isNull(iswhere)) {
					if (!iswhere.toUpperCase().contains("SELECT")
							&& !iswhere.toUpperCase().contains("FROM")) {
						sb.append("SELECT * FROM ");
	...
	public int[] createTables(List<String> sqlList) {
		...
		try {
			...
			state = connectionMaster.prepareStatement("");
	 ...
	 * List list = null; String table = ""; String isOrder = ""; SqlCommonDAO
	 * helper = new SqlCommonDAO(); if (!StringUtil.isNull(desc) &&
	 * "DESC".equals(desc.toUpperCase())) isOrder = "ORDER BY ROWNUM DESC";
	 * StringBuffer sb_sql = new StringBuffer();
	 * 
	 * try { if (connection == null || connection.isClosed()) { //
	 * connection=ConnectionFactory.getConnection(); connection =
	 * ConnectionFactory.getInstance().getConnection(); } boolean tbl =
	 * o.newInstance().getClass().isAnnotationPresent(Table.class);
	 * //System.out.println("isTable:" + tbl); if (tbl) { Table t = (Table)
	 * o.newInstance().getClass().getAnnotation(Table.class); //
	 * table=connection.getCatalog()+"."+t.name(); table = t.name(); }
	 * 
	 * helper.setConnection(connection); Command c = new Command();
	 * helper.setCommand(c); if (ConnectionFactory.EHCACHE) { c.setCache(true);
	 * } // if(p1>0&&p2>0){
	 * sb_sql.append("SELECT * FROM ").append("(").append(" SELECT A.*, ROWNUM RN "
	 * )
	 * .append(" FROM (SELECT * FROM  ").append(table).append("  ").append(isOrder
	 * ).append(" ) A")
	 * .append(" WHERE ROWNUM <= ").append(p2).append(" )").append
	 * (" WHERE RN >= ").append(p1); // } c.setTables(new String[] { table });
	 * c.setSql(sb_sql.toString()); //System.out.println("sql:" +
	 * sb_sql.toString()); Result rs = helper.executeQuery();
	 * if(rs!=null&&rs.getRowCount()>0) list = JDBCBeanUtil.transTOList(rs, o);
	 * //connection.close(); } catch (SQLException e) { // TODO Auto-generated
	 * catch block e.printStackTrace(); } catch (IllegalAccessException e) { //
	 ...
	 * helper.closeConnection(connection); } return list; }
	 */
	...
	public <E> E query(String sql, Class<E> maplistClass, String table,
		...
		try {
			...
			if (null != connection) {
				...
				if (rs != null && rs.next()) {
					if (maplistClass == Map.class) {
						...
						throw new RuntimeException("" + maplistClass
								+ " 该类型目前还没有做扩展!");
					}
				}
			}
			...
			log.error("SQL:" + sql, e);
			log.error("<E> E query方法多表联合操作调用异常："
					+ sql
					+ "\n"
					+ "---------------------queryTables方法调用-系统提示-----------------------"
	...
	public <T> List<T> queryList(Class<T> Clazz, String isWhere,
			boolean isCache, Object... obj) throws SQLException {
		String tableName = "";
		List<T> list = null;
		String sql = "";
		StringBuffer sb = new StringBuffer();
		...
		try {
			boolean tbl = Clazz.newInstance().getClass()
					.isAnnotationPresent(Table.class);
			if (tbl) {
				Table t = (Table) Clazz.newInstance().getClass()
						.getAnnotation(Table.class);
				tableName = t.name();
				if (StringUtil.isEmpty(tableName)) {
					log.error("bean映射表名异常:" + tableName);
	...
	public List<?> queryByList(Class<?> Clazz, String isWhere, boolean isCache,
		...
		if (null != connection) {
			...
			try {
				...
				if (rss != null && rss.getRowCount() > 0)
					list = JDBCBeanUtil.transTOList(rss, Clazz);
			} catch (IllegalAccessException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (InvocationTargetException e) {
	 ...
	 *            排序方式：ASC，DESC 任选其一
	 */
	public <T> void sortList(List<T> list, final String sortField,
		...
		Collections.sort(list, new Comparator<T>() {
			...
			public int compare(T o1, T o2) {
				try {
					...
					boolean ASC_order = (sortMode == null || "DESC"
	...
	public List<?> queryForList(Class<?> Clazz, String sql, Object... obj)
			throws SQLException {
		List<?> list = null;
		if (connection == null || connection.isClosed()) {
			// connection = ConnectionFactory.getInstance().getConnection();
	 ...
	 * @throws Exception
	 */
	private int saveForTransaction(Object obj, SqlCommonDAO helper)
			throws Exception {
		int i = 0;
		String tbl_name = "";
		String nameColum = "", valueColum = "";// 记录字段名
		...
		try {
			 ...
			 * finally { helper.closeConnection(connection); }
			 */
		...
		try {
			...
			try {
				if (null != multiSQL && multiSQL.size() > 0) {
					...
					for (Map.Entry<String, Object[]> entry : multiSQL
							.entrySet()) {
						// System.out.println("Key = " + entry.getKey() +
						// ", Value = " + entry.getValue());
						int c = helper.executeUpdate(entry.getKey(),
								connection, entry.getValue());
						if (c > 0) {
							i += c;
						} else {
							log.info("\n同一事物下，执行多条sql语句操作失败，错误信息："
									+ entry.getKey());
							connectionMaster.rollback();
							break;
						}
