...
public class MSlaveConnectionFactory_3db2 {
	private static final Logger logger = LoggerFactory.getLogger(MSlaveConnectionFactory_3db2.class);
...
/*	static {
	...
	private static final String             URL = "db03_slave_url";
	private static final String        USERNAME = "db03_slave_username";
	private static final String        PASSWORD = "db03_slave_password";
	
	//druid
	private static final String                            INITIALSIZE="db03_slave_initialSize";
	private static final String                                MINIDLE="db03_slave_minIdle";
	private static final String                              maxActive="db03_slave_maxActive";
	private static final String                                maxWait="db03_slave_maxWait";
	private static final String          timeBetweenEvictionRunsMillis="db03_slave_timeBetweenEvictionRunsMillis";
	private static final String             minEvictableIdleTimeMillis="db03_slave_minEvictableIdleTimeMillis";
	private static final String  maxPoolPreparedStatementPerConnectionSize="db03_slave_maxPoolPreparedStatementPerConnectionSize";
	private static final String                        validationQuery="db03_slave_validationQuery";
	private static final String                                filters="db03_slave_filters";
	private static final String                      AutoCommitOnClose="db03_slave_autoCommitOnClose";
	private static final String              maxOpenPreparedStatements="db03_slave_maxOpenPreparedStatements";
	private static final String                 PoolPreparedStatements="db03_slave_poolPreparedStatements";
	private static final String                           TestOnBorrow="db03_slave_testOnBorrow";
	private static final String                           TestOnReturn="db03_slave_testOnReturn";
	...
	public  static  boolean    db03_db_Slave_R =false;//默认从库为只读方式
	
	MSlaveConnectionFactory_3db2(){}    
	...
	static {
		...
		if(null!=BUNDLE){
			...
			try {
				...
				logger.error("警告:获取db03_db_Slave_R");
			}
	  if(Is_MS_OPEN&&db03_db_Slave_R){
			try {
				EncryptUtils.isGOTO();
			} catch (Exception e) {
			...
			try {
				EHCACHE   =StringUtil.StringToBoolean(BUNDLE.getString("db03_slave_ehcache"));
			} catch (Exception e1) {
		...
		//driud
		 try{
				...
				dd.setTestWhileIdle(StringUtil.StringToBoolean(BUNDLE.getString("db03_slave_testWhileIdle")));
	...
	public static synchronized Connection getConnection() {
	    ...
	    private static MSlaveConnectionFactory_3db2 instance;
	    
	    public static MSlaveConnectionFactory_3db2 getInstance() {
	        if (instance == null&&EncryptUtils.LOCK) {
	            synchronized (MSlaveConnectionFactory_3db2.class) {
	                if(instance == null) {
	                    instance = new MSlaveConnectionFactory_3db2();
	                }
	                logger.info("\n"+
							"=========================================================================\n"+
							"‖                                     从01-db03数据源实例化                                                                   ‖\n"+
