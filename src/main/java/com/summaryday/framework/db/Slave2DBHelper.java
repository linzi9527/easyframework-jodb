...
public class Slave2DBHelper {
	private static final Logger log = LoggerFactory.getLogger(Slave2DBHelper.class);
	private static final Slave2DBHelper postDB = new Slave2DBHelper();
	private Connection connection = null;
	private String[] link_name = null;

	private static IHotRedisData hotRedisData;//redis缓存操作对象

	public static Slave2DBHelper getInstance() {
	 ...
	 * 设置关联表名称即表名
	 */
	public Slave2DBHelper setTableLink(String[] tbl_name) {
	...
	public ResultSet executeQuery(String sql) {
		...
		try {
			if (connection == null || connection.isClosed()) {
				connection =Slave2ConnectionFactory.getInstance().getConnection();
				// connection=SlaveSlave2ConnectionFactory.getConnection();
			}
	...
	public List<?> queryList(Class<?> o, String isWhere, boolean isCache) {
		...
		try {
			...
			if (connection == null || connection.isClosed()) {
				// connection=Slave2ConnectionFactory.getConnection();
				connection = Slave2ConnectionFactory.getInstance().getConnection();
			}
			boolean tbl = o.newInstance().getClass().isAnnotationPresent(Table.class);
			//System.out.println("isTable:" + tbl);
	...
	public List<?> queryByOracle(Class<?> o, int p1, int p2, String desc) {
		...
		try {
			...
			Command c = new Command();
			helper.setCommand(c);
			if (Slave2ConnectionFactory.EHCACHE) {
				c.setCache(true);
			}
			// if(p1>0&&p2>0){
